...
위에서 언급하지 않았지만 `hooks`와 `event` 옵션은 에디터에서 사용자가 글을 작성할 때 특정한 이벤트가 발생할 경우에 작동하게할 수 있는 함수라고 보면 된다. 이는 다음 글 [Toast ui Editor 이미지 업로드하기](https://heomne.github.io/toast_ui_2)에서 자세히 다루도록 하겠다.
